...
> ⚠️ **「自动领券」目前只支持美团。** 其余平台没有可编程的领取接口，
> 只能手动把券数据喂进来 —— 计算部分照跑，不受影响。详见
> [平台支持现状](#平台支持现状)。
...
| 平台 | 自动领券 | 出清单 |
| --- | --- | --- |
| **美团** | ✅ 一键领取当日全部券，返回结构化的门槛 / 面额 / 品类 | ✅ |
| 淘宝闪购、京东外卖、饿了么、打车团购等 | ❌ 只有网页/小程序领券入口，得自己点进去领 | ✅ 把券贴进来即可 |

别家没有可编程的领取接口，本项目的做法也**不是去爬页面** —— 那些领券入口是活动 H5，
改一次版就要重写一次爬虫，爬来的数据还会因为登录态失效而悄悄变少，不值得。

所以美团之外**只能手动**：你把券列表粘过来（或给张截图），第 ② ③ ④ 步照跑。
计算部分与来源平台无关，只要字段对得上就能算。
...
第 ① 步是一条**取数通道**：本机挂了美团官方能力时就是「自动领券」；
没有对应能力、或者想算别家的券，就按路径 B 手动喂数据（见
[daily-coupon-list/references/data-sources.md](daily-coupon-list/references/data-sources.md)）。
第 ② ③ ④ 步是本项目的主体：纯本地计算，不联网、不上传，**与券来自哪个平台无关**。
...
技能本身只是「你问了它才做」。要做到名字里承诺的**每天自己领、自己出清单**，
把它挂到 WorkBuddy 的[自动化](https://www.workbuddy.cn/docs/workbuddyapp/features/Auto)上，每天跑一次即可。

**建任务：** 侧边栏 →「自动化」→ 添加定时任务（`+`），填四项：
...
| 提示词 | 下面那段，直接复制 |
| 执行频率 | 每天 |
| 执行时间 | `09:00`（当日券池刚刷新，且离到期还有一整天） |

> 在「技能」里勾上 `daily-coupon-list`，或靠提示词里的关键词触发都行。
> 也支持「对话创建」——直接说「每天早上 9 点领美团券并输出券后省钱清单」，
> 它会自己解析成一条定时任务。
...
1. 走美团的取数通道领取当日全部券，把返回落盘成 JSON 文件
   （不要直接读命令行管道，Windows 下中文会被按 GBK 解码成乱码）。
2. 跑 daily-coupon-list 技能的计算脚本生成清单：
   python scripts/minpay.py --file <刚落盘的 JSON>
3. 在最前面补一句一句话结论，然后把脚本输出的 Markdown 清单原样发我。

硬约束：
- 返回码非 0（例如「您今天已经领取过优惠券」）不算失败，coupons[] 照旧是齐的，继续解析。
- 取数失败（未登录 / 无 token）时只输出「需要重新登录」和扫码步骤，
  禁止改用网页抓取、禁止编造券名或金额。
- 券名、门槛、面额必须来自真实返回，缺失写「未返回」，不推测不补数。
- 不要做商品搜索。
- A 组 / B 组为 0 张是常态，如实说明，不要放宽判据凑数。
- 自动领券只对美团成立，其他平台不要改成去爬活动页。
```

建完先点一次「**测试运行**」 —— 首次大概率会撞上登录态，照提示扫一次码就好。
想结果直接到手机，可以再开「推送到小程序 / 企业微信 bot」，不用开客户端。

> ⚠️ 自动化要求「本机有美团的取数通道 + 登录态有效」。
> 每次券池刷新时间、各平台登录有效期都可能变，失败时任务会明说「需要重新登录」，
> 而不是给你一份编造的清单 —— 这是刻意写死的。
...
- **自动领券目前只支持美团**；其他平台没有可编程入口，需要你手动提供券数据。
  本项目不提供、也不计划提供针对其他平台的页面抓取。
